...
// Current index in text input
static unsigned char inputLocation = 0;

// Finds value of input char array, relatively small and fast I think.
static double calc(const char input[CALC_BUFFER_SIZE +1])
{
  // Input buffer, used when a single token (generally a number) takes up more.
  char inputToken[CALC_BUFFER_SIZE + 1];

  // Keep track of indices
  unsigned char inputTokenLocation = 0, inputLocation = 0;

  // Input, converted to tokens, one extra large to accomodate for possible negative sign then open parenthesis as first character.
  struct Token tokens[CALC_BUFFER_SIZE + 1];

  // Keep track of index
  unsigned char tokenCount = 0;

  // Kind of a hacky solution to determining whether to treat a dash as a minus sign or a negative sign.
  bool dashAsMinus = false;

  // Characters limitation (32 by default).
  while(inputLocation < CALC_BUFFER_SIZE + 1)
  {
    char digit = input[inputLocation];

    // Determine if dash need to be considered as minus.
    ...
    // If valid digit or valid floating point or - as negative sign
    if ((digit >= '0' && digit <= '9') ||
        (inputTokenLocation != 0 && input[inputLocation] == CALC_CHAR_DEC) ||
        (!dashAsMinus && inputTokenLocation == 0 && input[inputLocation] == CALC_CHAR_SUB))
    {
    ...
    // If not the first characters (?)
    if(inputTokenLocation != 0)
    {
      // I would like to use sscanf here, but the small version of stdio.h on the chip doesn't allow sscanf or its sister functions to be used to process floats.
...
bool process_record_user(uint16_t keycode, keyrecord_t* record)
{
    ...
    // If layer calc and set as numlock.
    ...
    switch(action) {
      ...
      // 0 when nothing should happen and QMK should work as usual
      case 0:
          return true;

      // -1 when invalid input was given, QMK should ignore it
      case -1:
          return false;

      // -2 when BSP should be done
      case -2:
          if(inputLocation > 0)
          {
              inputLocation--;
